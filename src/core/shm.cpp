...
static const struct wl_shm_interface shm_impl = { .create_pool = &barock::shm_t::handle_create_pool, .release = &barock::shm_t::handle_release };
...
void
shm_t::handle_create_pool(wl_client *client, wl_resource *resource, uint32_t id, int32_t fd, int32_t size) {
  ...
  wl_resource *shm = wl_resource_create(client, &wl_shm_pool_interface, wl_resource_get_version(resource), id);
