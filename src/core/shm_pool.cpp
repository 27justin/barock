...
struct wl_buffer_interface wl_buffer_impl {
  .destroy = [](wl_client *, wl_resource *buffer) {
    barock::shm_pool_t &pool = *reinterpret_cast<barock::shm_pool_t*>(wl_resource_get_user_data(buffer));
    ERROR("Destroy SHM buffer");
  }
};

void
create_buffer(struct wl_client *client, struct wl_resource *wl_shm_pool, uint32_t id, int32_t offset, int32_t width, int32_t height, int32_t stride, uint32_t format) {
  ERROR("shm pool :: create buffer");

  wl_resource *buffer = wl_resource_create(client, &wl_buffer_interface, wl_resource_get_version(wl_shm_pool), id);
  if (!buffer) {
  ...
  buf->stride = stride;
  buf->format = format;
  buf->data = reinterpret_cast<void *>(
    reinterpret_cast<uintptr_t>(buf->pool->data) + offset
    );

  INFO("create buffer\n  offset: {}\n  width: {}\n  height: {}\n  stride: {}\n  format: {}", offset, width, height, stride, format);
