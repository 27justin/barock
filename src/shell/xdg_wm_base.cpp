...
void
xdg_shell_t::handle_xdg_base_get_surface(wl_client *client, wl_resource *wm_base_resource, uint32_t id, wl_resource *surface_result) {
  xdg_shell_t *shell = reinterpret_cast<xdg_shell_t *>(wl_resource_get_user_data(wm_base_resource));
  ...
  struct wl_resource *xdg_surface_resource = wl_resource_create(
    ...
    wl_resource_get_version(wm_base_resource),
    id
  );
  ...
  wl_resource_set_implementation(xdg_surface_resource, &xdg_surface_impl, nullptr, &barock::xdg_shell_t::xdg_surface_destroy);

  // Send the configure event
  xdg_surface_send_configure(xdg_surface_resource, wl_display_next_serial(shell->compositor.display()));
